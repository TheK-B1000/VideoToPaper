...
def test_main_assemble_paper_writes_artifact_manifest_without_audit(
    tmp_path: Path,
) -> None:
    ...
    assert "Paper artifact manifest written to:" in result.stdout
    ...
    assert artifacts["paper_spec"]["path"] == str(paper_spec_output_path)
    assert artifacts["paper_spec"]["sha256"] == _sha256(paper_spec_output_path)

    assert artifacts["html_paper"]["exists"] is True
    assert artifacts["html_paper"]["path"] == str(html_output_path)
    assert artifacts["html_paper"]["sha256"] == _sha256(html_output_path)

    assert artifacts["paper_run_report"]["exists"] is True
    assert artifacts["paper_run_report"]["path"] == str(run_report_path)
    assert artifacts["paper_run_report"]["sha256"] == _sha256(run_report_path)

    assert "html_audit_report" not in artifacts


def test_main_assemble_paper_writes_artifact_manifest_with_audit(
    tmp_path: Path,
) -> None:
    source_registry_path = tmp_path / "source_registry.json"
    claim_inventory_path = tmp_path / "claim_inventory.json"
    ...
    assert artifacts["paper_spec"]["sha256"] == _sha256(paper_spec_output_path)
    assert artifacts["html_paper"]["sha256"] == _sha256(html_output_path)
    assert artifacts["html_audit_report"]["sha256"] == _sha256(audit_report_path)
    assert artifacts["paper_run_report"]["sha256"] == _sha256(run_report_path)
