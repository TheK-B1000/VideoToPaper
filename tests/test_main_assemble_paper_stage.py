...
def test_main_assemble_paper_stage_writes_spec_and_html(tmp_path: Path) -> None:
    ...
    assert "Custom Inquiry Paper" in html
...
def test_main_assemble_paper_stage_fails_when_upstream_artifact_is_missing(
    tmp_path: Path,
) -> None:
    claim_inventory_path = tmp_path / "claim_inventory.json"
    evidence_integration_path = tmp_path / "evidence_integration.json"
